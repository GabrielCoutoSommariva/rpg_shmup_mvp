...
class HubView(arcade.View):
    def __init__(self, window):
        ...
        self.speed = 220
        # Use a SpriteList for drawing (compatibility across Arcade versions)
...
class ArenaView(arcade.View):
    def __init__(self, window):
        ...
        self.player = Player(arcade.SpriteSolidColor(20, 20, arcade.color.SPRING_BUD))
    ...
    def on_update(self, dt):
        ...
        if arcade.MOUSE_BUTTON_LEFT in self.window.pressed_mouse:
            ...
            for x, y, vx, vy in make_bullets("straight", (px, py), 300, angle=angle):
                spr = arcade.SpriteSolidColor(4, 4, arcade.color.YELLOW); spr.center_x, spr.center_y = x, y
                self.bullet_sprite_list.append(spr); self.bullets.append((spr, vx, vy))

        # update bullets (safe rebuild)
        new_bullets = []
        for spr, vx, vy in self.bullets:
            spr.center_x += vx*dt; spr.center_y += vy*dt
        ...
        for ev in self.wave.update(dt):
            for entry in ev["spawn"]:
                for _ in range(entry["n"]):
                    ...
                    self.enemies.append(e); self.enemy_sprite_list.append(e.sprite)
        ...
        for e in list(self.enemies):
            ...
            if e.ready():
                ...
                for x, y, vx, vy in make_bullets(e.pattern, (e.sprite.center_x, e.sprite.center_y), 180, angle=angle):
                    ...
                    self.enemy_bullet_sprite_list.append(spr); self.enemy_bullets.append((spr, vx, vy))
        ...
        for spr, vx, vy in list(self.bullets):
            ...
            if hits:
                for h in hits:
                    for e in list(self.enemies):
                        if e.sprite is h: self.enemies.remove(e); break
        ...
        if arcade.check_for_collision_with_list(self.player.sprite, self.enemy_bullet_sprite_list):
            ...
            for _ in range(min(5, len(self.enemy_bullets))):
                spr, vx, vy = self.enemy_bullets.pop(0); spr.remove_from_sprite_lists()
            if self.player.hp <= 0: self.window.show_view(HubView(self.window))

        if self.time >= 90: self.window.show_view(HubView(self.window))
...
class GameWindow(arcade.Window):
    def __init__(self):
        ...
        self.pressed_keys=set(); self.pressed_mouse=set(); self.mouse_xy=(0,0)
